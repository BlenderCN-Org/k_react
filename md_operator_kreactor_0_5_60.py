...
class FaceKreactor(bpy.types.Operator):
    bl_idname = "md.k_reactor_face"
    ...
    def invoke(self, context, event):
        ...
        self.timer = context.window_manager.event_timer_add(REFRESH_INTERVAL/1000., context.window)
        ...
        if(self.object != None):
            if(solution[4] != FACE_NONE):
                if(solution[4] == FACE_FILL):
                    bmesh.ops.delete(self.bm, geom=[solution[2]], context=3)
                elif(solution[4] == FACE_SNAP):
                    delete_edge = [e for e in solution[2].edges if not(e in solution[0])]
                    bmesh.ops.delete(self.bm, geom=delete_edge, context=2)
                    ...
                    bmesh.ops.delete(self.bm, geom=delete_verts, context=1)
                    ...
                    if(solution[0] != []):
                        bmesh.ops.delete(self.bm, geom=solution[0], context=2)
                    elif(solution[1] != []):
                        bmesh.ops.delete(self.bm, geom=solution[1], context=1)
        ...
        if(self.hit_location != None):
            ...
            if(closest_vertex_distance < average_edge_length * SNAP_RATIO):
                if(self.hold_line_vertex != None and  not self.snap_mode and self.active_vertex != None
                    ...
                    bmesh.ops.delete(self.bm, geom=[self.active_vertex], context=1)
                    self.active_vertex = None
                ...
                if(self.snap_mode):
                    if(self.snap_line != None):
                        bmesh.ops.delete(self.bm, geom=[self.snap_line], context=2)
                    self.active_vertex = self.bm.verts.new(self.hit_location)
...
def register():
    bpy.utils.register_class(FaceKreactor)
    #bpy.utils.register_class(FaceKreactorHelp)

def unregister():
    bpy.utils.unregister_class(FaceKreactor)
    #bpy.utils.unregister_class(FaceKreactorHelp)
